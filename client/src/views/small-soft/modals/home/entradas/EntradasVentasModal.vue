...
watch(
  () => [puntoVentaStore.modalEntradasVentas],
  async ([value]) => {
    console.log(value);
...
watch(isDialogVisible, async (newValue) => {
  ...
  if (!newValue) {
    puntoVentaStore.modalEntradasVentas = newValue;
  }
});
</script>
...
<template>
  ...
  >
    ...
    <!-- Dialog Content -->
    <VCard title="Registro de entradas de dinero">
      <VRow>
        ...
        <VCol cols="12" md="4" sm="6">
          <VCardText class="mt-7">
            <VRow>
              <VCol cols="12">
                <h4 class="text-muted text-center">Detalle de la Entrada</h4>
              </VCol>
              <VCol cols="12">
                <AppTextField
                  v-model="password"
